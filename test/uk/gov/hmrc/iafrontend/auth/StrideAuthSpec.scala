...
class StrideAuthSpec extends ITSpec with authMock {
  ...
  "StrideAuthenticatedAction should Allow a valid authorized request " in {
  ...
  "StrideAuthenticatedAction should Allow a valid authorized request and be case insensitive" in {
    mockAuthorise(AuthProviders(PrivilegedApplication), Retrievals.allEnrolments)(Future.successful(Enrolments(Set(Enrolment("insolvency_analytics_user")))))
    val result: Result = testAuthRequest.invokeBlock(fakeRequestGet, (test: Request[Product with Serializable]) => Future.successful(Ok(""))).futureValue
    status(result) shouldBe 200
  }

  "StrideAuthenticatedAction should return an unauthorized " in {
  ...
  "StrideAuthenticatedAction should redirect to stride login page if the requestor is not logged in " in {
