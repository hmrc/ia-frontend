...
class LockSpec extends ITSpec {
  ...
  "happy path" in {
    WireMockResponses.acquireLock(timeout)
    ...
    WireMockResponses.verifyAcquireLock(timeout)
    WireMockResponses.verifyReleaseLock()
  }

  "fail case when lock is already in use" in {

    WireMockResponses.acquireExistingLock(timeout)
